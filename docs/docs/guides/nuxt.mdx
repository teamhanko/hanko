...
:::info
This guide and the nuxt-hanko module are only compatible with Nuxt 3. Find out more about Nuxt at [Nuxt.com](https://nuxt.com).
:::
...
A global server middleware is added by `nuxt-hanko`. After decoding and validating the JWT for the request, a new `hanko` property to is added to the event context.
