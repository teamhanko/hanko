...
Currently, we do not provide an SDK that makes the integration as easy as the [Hanko element web component](/guides/frontend).
...
You will either get an HTTP status code of `200` or `409`. If you got a `409`, then the user already exists, and you can obtain the users `id` from the success response of the endpoint mentioned in the [previous step](#check-if-user-exist).
...
You need to provide the user's `id` you obtained in one of the previous steps. If sending the passcode was successful, you will get an `id` for the passcode in the response object. Remember this `id`, because you will need it to finalize the passcode.
...
provide for their systems. Find out how to use them [here](https://developer.apple.com/documentation/authenticationservices/public-private_key_authentication/supporting_passkeys)
