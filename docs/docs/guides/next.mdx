...
`<hanko-auth>` web component adds login interface to your app. Begin by importing the register function from `@teamhanko/hanko-elements` into your Next.js component. Call it with the Hanko API URL as an argument to register `<hanko-auth>` to the browser's [CustomElementRegistry](https://developer.mozilla.org/de/docs/Web/API/CustomElementRegistry). Once done, include it in your JSX.
...
:::info
The Hanko client from @teamhanko/hanko-elements lets you "listen" for specific events, like when a user logs in.
:::
...
The `<hanko-profile>` component offers a page for managing email addresses and passkeys.
...
You'll use `@teamhanko/hanko-elements` to manage user logouts. When a user logs out, a specific event is triggered that you can subscribe to. Below, we make a logout button component that you can use anywhere.
...
To add JWT verification middleware with Hanko in your Next.js application, begin by installing the [jose library](https://www.npmjs.com/package/jose). This middleware will ensure secure access to specific routes, like `/dashboard` here, by checking for valid JWT tokens. Here we define the Hanko API URL, extract and verify the JWT from cookies, and redirect unauthorized users to the login page.
