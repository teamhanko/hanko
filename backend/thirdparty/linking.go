...
func LinkAccount(cfg *config.Config, p persistence.Persister, userData *UserData, providerName string) (*AccountLinkingResult, error) {
	...
	if identity == nil {
		return signUp(p, userData, providerName)
	} else {
		return signIn(cfg, p, userData, identity)
	}
}

func signIn(cfg *config.Config, p persistence.Persister, userData *UserData, identity *models.Identity) (*AccountLinkingResult, error) {
	var linkingResult *AccountLinkingResult
	err := p.Transaction(func(tx *pop.Connection) error {
		...
		if userData.Metadata.Email != identity.Email.Address {
			...
			if terr != nil {
				return ErrorServer("could not get email").WithCause(terr)
			}
			...
			if email != nil {
				if email.UserID == nil {
					...
					if terr != nil {
						return ErrorServer("could not update email").WithCause(terr)
					}
					...
					return ErrorMultipleAccounts(fmt.Sprintf("cannot identify associated user: '%s' is used by multiple accounts", email.Address))
				} else {
				...
				if err != nil {
					return ErrorServer("failed to count user emails").WithCause(err)
				}

				if emailCount >= cfg.Emails.MaxNumOfAddresses {
					return ErrorMaxNumberOfAddresses("max number of email addresses reached")
				}
				...
				if terr != nil {
					return ErrorServer("could not create email").WithCause(terr)
				}
		...
		if terr != nil {
			return ErrorServer("could not get identity").WithCause(terr)
		}

		user, terr := userPersister.Get(*identity.Email.UserID)
		if terr != nil {
			return ErrorServer("could not get user").WithCause(terr)
		}
...
func signUp(p persistence.Persister, userData *UserData, providerName string) (*AccountLinkingResult, error) {
	var linkingResult *AccountLinkingResult
	err := p.Transaction(func(tx *pop.Connection) error {
		userPersister := p.GetUserPersisterWithConnection(tx)
		emailPersister := p.GetEmailPersisterWithConnection(tx)
		...
		if email != nil && email.UserID != nil {
			return ErrorUserConflict("third party account linking for existing user with same email disallowed")
		}

		user := models.NewUser()
		terr = userPersister.Create(user)
		if terr != nil {
			return ErrorServer("could not create user").WithCause(terr)
		}
		...
		if email != nil && email.UserID == nil {
			...
			terr = emailPersister.Create(*email)
			if terr != nil {
				return ErrorServer("failed to store email").WithCause(terr)
			}
		}
		...
		if terr != nil {
			return ErrorServer("failed to store primary email").WithCause(terr)
		}

		identity, terr := models.NewIdentity(providerName, userData.ToMap(), email.ID)
		if terr != nil {
			return ErrorServer("could not create identity").WithCause(terr)
		}
