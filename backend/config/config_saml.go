...
type Saml struct {
	...
	Endpoint string `yaml:"endpoint_url" json:"endpoint_url" koanf:"endpoint_url"`
	// `audience_uri` determines the intended recipient or audience for the SAML Assertion.
	AudienceUri string `yaml:"audience_uri" json:"audience_uri" koanf:"audience_uri"`
	// `default_redirect_url` is the URL to redirect to in case of errors or when no `allowed_redirect_url` is provided.
	DefaultRedirectUrl string `yaml:"default_redirect_url" json:"default_redirect_url" koanf:"default_redirect_url"`
	...
	IdentityProviders []IdentityProvider `yaml:"identity_providers" json:"identity_providers" koanf:"identity_providers"`
}

type Options struct {
	// `sign_authn_requests` determines whether initial requests should be signed.
	SignAuthnRequests bool `yaml:"sign_authn_requests" json:"sign_authn_requests" koanf:"sign_authn_requests" jsonschema:"default=true"`
	// `force_login` forces the IdP to always show a login (even if there is an active session with the IdP).
	ForceLogin bool `yaml:"force_login" json:"force_login" koanf:"force_login" jsonschema:"default=false"`
	// `validate_encryption_cert` determines whether the certificate used for the encryption of the IdP responses should
	// be checked for validity.
	ValidateEncryptionCertificate bool `yaml:"validate_encryption_cert" json:"validate_encryption_cert" koanf:"validate_encryption_cert" jsonschema:"default=true"`
	// `skip_signature_validation` determines whether the validity check of an IdP response's signature
	// should be skipped.
	SkipSignatureValidation bool `yaml:"skip_signature_validation" json:"skip_signature_validation" koanf:"skip_signature_validation" jsonschema:"default=false"`
	...
	AllowMissingAttributes bool `yaml:"allow_missing_attributes" json:"allow_missing_attributes" koanf:"allow_missing_attributes" jsonschema:"default=false"`
}
...
type IdentityProvider struct {
	...
	MetadataUrl string `yaml:"metadata_url" json:"metadata_url" koanf:"metadata_url"`
	// `skip_email_verification` determines whether the check if the `email_verified` attribute in the IdP response
	// will be skipped.
	SkipEmailVerification bool `yaml:"skip_email_verification" json:"skip_email_verification" koanf:"skip_email_verification"`
	// `attribute_map` is a map of attributes used to map attributes in IdP response to custom attributes at
	// Hanko.
	AttributeMap AttributeMap `yaml:"attribute_map" json:"attribute_map" koanf:"attribute_map" jsonschema:"title=attribute_map"`
}
